...
lossSaveFile = "Losses_ModelNew31.txt" # needs to be manually cleared, as is just added to by this program
inputImageType = 'RGB'
...
# ModelNew31.printLearningRate(ModelNew31.optimizerD)
def printLearningRate(optimizer):
		print(getLearningRate(optimizer))
		
# ModelNew31.updateLearningRate(ModelNew31.optimizerD, 0.0001)
def updateLearningRate(optimizer, learningRate):
	for g in optimizer.param_groups:
		g['lr'] = learningRate

# versions of the above functions for easier use
# ModelNew31.updateLearningRateThisModel(0.0001)
...
def train(net, netD, optimizerG, optimizerD, features, labels, training=True, useNoise=False, device='cpu', trainGen=True, trainDis=True, vgg=True):
	...
	for _ in range(criticPerGenerator):
		if not useNoise:
			...
			noise = torch.randn((features.size(dim=0)), channels, inSideSize, inSideSize, device=device)
			outG = net(noise)
		outD_real = netD(labels)
		outD_real = reshape(outD_real, (-1, ))
		outD_fake = netD(outG).reshape(-1)
		outD_fake = reshape(outD_fake, (-1, ))
		gp = gradientPenalty(netD, labels, outG, device=device)
	...
	#VGG19 feature part
	if (vgg):
		sr_imgs_in_vgg_space = truncated_vgg19(outG)
		hr_imgs_in_vgg_space = truncated_vgg19(labels).detach()
...
def trainNotGan(net, optimizerG, features, labels, training=True, useNoise=False, device='cpu'):
	...
	if not useNoise:
		...
		noise = torch.randn((features.size(dim=0)), channels, inSideSize, inSideSize, device=device)
		outG = net(noise)
	adversarial_loss = content_loss_criterion(outG, labels)
	#print(str(content_loss.item()) + "\n")
	#print(str(adversarial_loss.item()) + "\n")
...
# ModelNew31.train_DeepLearn2(ModelNew31.model, ModelNew31.modelD, [ModelNew31.Image.open("/home/trace/Pictures/TestFolder/TestImage.png")], ModelNew31.optimizerG, ModelNew31.optimizerD, True)
...
# ModelNew31.train_DeepLearnOnDirectory("dataset/train2014/", ModelNew31.model, ModelNew31.modelD, ModelNew31.optimizerG, ModelNew31.optimizerD, True, 1, 0, 0, True, True, True, True, True)
...
# ModelNew31.train_DeepLearnOnDirectoryRepeat("dataset/train2014/", ModelNew31.model, ModelNew31.modelD, ModelNew31.optimizerG, ModelNew31.optimizerD, True, 1, 16, True, True, True, True, False)
...
def useOnPILImage(inputImage, cap=False, normalize=False):
	...
	tensorTemp = torch.zeros_like(inputImage)
	tensorTemp3 = torch.tensor([tensorTemp.tolist(), inputImage.tolist()])
	tensorTemp2 = tensorTemp3.to(device)

	output = model(tensorTemp2) # gets output from machine
	output = output[1] # only grabs the result from the inputImage
...
# ModelNew31.makeUpscale("TestFolder/563.jpg", cap=True) is an input for testing
...
# ModelNew31.makeUpscaleOfFullImage("/home/trace/Pictures/TestFolder/TestImage.png", cap=True) is an input for testing
