...
path1 = input("Path to half sized images: ")
path2 = input("Path to full sized images: ")
folder1 = input("Path to place half sized squares: ")
folder2 = input("Path to place full sized squares: ")
...
while tempNum < repeats:
	for p in range(len(files1)): # assumes files1 and files2 are of same size as they were the corresponding images of each other, so p works for both
		...
		if (len(pixels1[x, y]) == 4): # for RGBA images
			...
			square2[square2Middle][square2Middle] = pixels2[x * 2, y * 2]
			...
			square2[square2Middle][square2Middle] = pixels2[x * 2, y * 2][0], pixels2[x * 2, y * 2][1], pixels2[x * 2, y * 2][2], defaultAlpha
		...
		while (temp1 < square2Size): # goes through the whole matrix in row then column
			...
			while (temp2 < square2Size):
				if ((x * 2 + tempx) < img2.size[0] and (y * 2 + tempy) < img2.size[1]):
					if ((x * 2 + tempx) >= 0 and (y * 2 + tempy) >= 0):
						if (len(pixels2[x + tempx, y + tempy]) == 4): # for RGBA images
							square2[temp1][temp2] = pixels2[x * 2 + tempx, y * 2 + tempy]
						else: # for taking input from things that are only RGB
							square2[temp1][temp2] = pixels2[x * 2 + tempx, y * 2 + tempy][0], pixels2[x * 2 + tempx, y * 2 + tempy][1], pixels2[x * 2 + tempx, y * 2 + tempy][2], defaultAlpha
				tempx = tempx + 1
				temp2 = temp2 + 1
			tempy = tempy + 1
			temp1 = temp1 + 1
